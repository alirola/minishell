...
/*   Updated: 2024/04/02 13:35:16 by alirola-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (node->full_cmd[0] && ft_strncmp(node->full_cmd[0], "pwd\0", 4) == EXIT_SUCCESS)
		pwd_exe();
	if (node->full_cmd[0] && ft_strncmp(node->full_cmd[0], "env\0", 4) == EXIT_SUCCESS)
		env_exe(d);
	if (node->full_cmd[0] && ft_strncmp(node->full_cmd[0], "unset\0", 6) == EXIT_SUCCESS)
		unset_exe(d, node->full_cmd[1]);
	if (node->full_cmd[0] && ft_strncmp(node->full_cmd[0], "cd\0", 3) == EXIT_SUCCESS)
		cd_exe(d,node->full_cmd, 0);
	if (node->full_cmd[0] && ft_strncmp(node->full_cmd[0], "export\0", 7) == EXIT_SUCCESS)
		export_exe(d, node->full_cmd, 1);
	if (node->full_cmd[0] && ft_strncmp(node->full_cmd[0], "exit\0", 5) == EXIT_SUCCESS)
	{
